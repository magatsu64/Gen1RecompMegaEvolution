...
- 53 more Mega records covering the rest of the real X/Y, ORAS, and
  Z-A roster outside Kanto (Meganium, Feraligatr, Skarmory, Froslass,
  Emboar, Excadrill, Scolipede, Scrafty, Eelektross, Chandelure,
  Chesnaught, Delphox, Greninja, Pyroar, Malamar, Barbaracle, Dragalge,
  Hawlucha, Eternal Flower Floette, Drampa, Falinks, Zygarde Complete
  Form, Ampharos, Steelix, Scizor, Heracross, Houndoom, Tyranitar,
  Sceptile, Swampert, Gardevoir, Sableye, Mawile, Aggron, Medicham,
  Manectric, Sharpedo, Camerupt, Altaria, Banette, Absol, Glalie,
  Salamence, Metagross, Latias, Latios, Rayquaza, Lopunny, Garchomp,
  Lucario, Abomasnow, Gallade, Audino, Diancie). Same status as
  BLAZIKEN_MEGA: verified real stats/typing, data-only, no art, no
  base species -- untestable without a separate mod providing the real
  base species first. 22 of these needed a typing fallback (no
  Fairy/Steel/Dark without the fairy_dark_steel_types dependency).
- Real art for Mega Blaziken, replacing its placeholder.
- Summary/stats screen (PKMN -> STATS mid-battle) now shows the real
  Mega-boosted Attack/Defense/Speed/Special and the Mega sprite while
  actively Mega Evolved. Previously read the permanent, unboosted mon
  record and a sprite cached when the screen opened -- neither aware
  of the separate battle-only fields Mega Evolution actually sets. HP
  is deliberately left alone, matching every real game -- Mega
  Evolution has never changed HP in any generation.

### Fixed

- Crash: every data-only (non-Kanto) species was missing
  spriteFront/spriteBack, both required fields in the schema. All 54
  now point at one shared, obviously-placeholder image instead of the
  recolor-derived path the crash came from trying to build, since that
  path fundamentally can't work for species with no vanilla sprite in
  this Kanto-only engine to recolor in the first place.
- The stats-screen sprite override was resolving the wrong path
  (a raw mod-relative string instead of running it through the same
  resolver every other sprite lookup in this mod already uses) --
  silently kept the old sprite showing even after the stats fix above
  landed correctly.
- Confirmed (not changed -- verified directly): Mega Evolution cannot
  persist outside of battle. Nothing in this mod or the engine's own
  save-serialization code ever writes the battle-only stat/type/sprite
  fields back onto a Pokemon's permanent record or into a save file.
