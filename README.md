...
## The art (placeholder, as agreed)

14 of the 21 forms are recolors of each species' own real vanilla
battle sprite (`transforms.lua`), not the actual Mega designs. Most
real Mega forms have genuine shape changes -- extra limbs, wings, armor
plating -- that a palette shift can't produce; this is a disclosed
placeholder, the same tradeoff these species had before their real art
arrived. Charizard X, Charizard Y, Venusaur, Blastoise, Raichu X,
Raichu Y, and Clefable all have real custom art now (all supplied by
you).
...
applied blindly twice.** Charizard X/Y, Venusaur, and Blastoise all
arrived at full front-matching resolution (56x56 back sprites, not the
traditional half-size) and need the `battle_sprite_scales` override
(`fullResBack = true` in the data table) the same way Charizard X
originally did. Raichu X/Y and Clefable's back sprites arrived at
28x28 -- exactly half of their 56x56 fronts, the *traditional* Gen 1
back-sprite convention -- so those get the engine's normal default 2x
back-scale like a recolor would, no override at all. Checked each
one's actual pixel dimensions before deciding, rather than assuming
every piece of custom art needs the same treatment.

None of the 14 recolors needed a `battle_sprite_scales` override
either, for the same reason as Raichu/Clefable: a recolor doesn't
resize the image, so each one is still the exact pixel dimensions of
its vanilla source, which the engine's default 2x back-scale already
renders correctly. Only 4 of the 7 real-art forms (Charizard X/Y,
Venusaur, Blastoise -- the full front-matching-resolution ones) needed
that override; Raichu X/Y and Clefable's traditional half-resolution
back sprites didn't.
...
- **catchRate/baseExp/growthRate/level1Moves** are uniform placeholders
  across all 21 forms (45 / 200 / MEDIUM_SLOW / Tackle) -- not
  researched per species, since none of these fields are ever read for
  a species that's never caught, leveled up, or taught a move here.
...
- `main.lua` -- all 21 species records and the generalized
  `BattleState.update`/`drawTextArea`/`draw` wraps
- `transforms.lua` -- the 14 remaining recolor recipes (front+back per
  form)
- `assets/charizard_x_front.png`, `assets/charizard_x_back.png`,
  `assets/charizard_y_front.png`, `assets/charizard_y_back.png`,
  `assets/venusaur_front.png`, `assets/venusaur_back.png`,
  `assets/blastoise_front.png`, `assets/blastoise_back.png`,
  `assets/raichu_x_front.png`, `assets/raichu_x_back.png`,
  `assets/raichu_y_front.png`, `assets/raichu_y_back.png`,
  `assets/clefable_front.png`, `assets/clefable_back.png` -- the seven
  sets of real custom art
